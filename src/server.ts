...
class LarkBotRuntime {
  /**
   * One WSClient per (spaceId, provider) pair. Two spaces can bind the same
   * Lark app id, so keying on provider alone would leak sessions across
   * spaces. The Lark app itself gets one WS connection per space here — that
   * matches the fact that channels/bindings are space-owned.
   */
