...
> 分支: feature_agents_daily  
> 范围限定: 单日 1 名实现 subagent 可完成；优先安全/稳定性硬伤，避免大型架构改动

## 选题逻辑（简短）
- `issue.md` 中 P0（API 鉴权 / 全量状态泄露）需要全局改造，单日落地风险高，今日不动。
- 选择 P1 中两项**改动局部、可单元验证**的安全 / 持久化修复，其余 issue 留待后续每日推进。
- `requirements.md` 中的需求（HOTL 审批、MCP Apps、Worktree）均为多日工程，今日不开新坑。

## 今日 TODO

### [TODO-1] 修复 workspace.ts 生成的 wrapper 脚本中的 Shell 注入风险
- **来源**: `issue.md` → P1 「生成 Agent 脚本时的 Shell 注入风险」
- **目标文件**: `src/workspace.ts`（核心改动点位 ~L112，函数 `prepareAgentWorkspace`）
- **要求**:
  ...
  - 在文件顶部或函数注释里加 1 行说明（不要长篇 docstring）："Shell-escape all interpolated values to prevent injection."
- **验收**:
  - `bash -n` 静态语法检查通过。
  - `npm run typecheck` 或 `tsc --noEmit`（若 `package.json` 已配置）通过 —— 若环境无 npm 依赖，跳过但需在 PR 描述里标注。
  - 若变量包含 `';rm -rf /;#`，转义后写入到脚本中的字面值仍是这串，不会被 shell 解释执行。
- **不要做**:
  - 不要重构 wrapper 模板格式或迁移到 JSON 配置（另立 issue）。
  - 不要触碰 runtime profiles 的 argument injection（同名 P1 但属另一改动点，今日不做）。

### [TODO-2] `JsonStore.persist` 改为原子写入（temp file + rename）
- **来源**: `issue.md` → P1 「同步且非原子的状态持久化机制」
- **目标文件**: `src/store/json-store.ts`（`persist` / `writeFileSync` 调用点 ~L28）
- **要求**:
  - 写入流程改为：
...
- **不要做**:
  - 不引入新 npm 依赖（如 `write-file-atomic`），用原生 `fs` 即可。
...
- 不要 push、不要建 MR（由主控统一处理）。

## 完成回执
实现 subagent 完成后请回报：
1. 实际修改的文件列表与每个文件的 `git diff --stat`。
2. 是否运行了 `tsc --noEmit` / `npm run build`，结果如何（环境无依赖时如实标注 skipped + 原因）。
3. 主要决策点与未解决疑问（如有）。
