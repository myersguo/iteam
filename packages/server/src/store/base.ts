...
import { clone, createId, nowIso } from "@iteam/shared";
import type {
  Agent,
  Channel,
  Computer,
...
import type { IStore, StateListener, StateMutator, MutateOptions } from "./types.js";
...
export function sanitizeState(state: State): State {
  ...
  state.externalMessageLinks = (state.externalMessageLinks || []).map((link: ExternalMessageLink) => ({
    ...
    externalThreadId: link.externalThreadId ?? null,
    externalRootMessageId: link.externalRootMessageId ?? null,
    externalParentMessageId: link.externalParentMessageId ?? null,
    externalReplyToMessageId: link.externalReplyToMessageId ?? null,
...
export abstract class BaseStore implements IStore {
  ...
  mutate<T>(fn: StateMutator<T>, options: MutateOptions = {}): T {
    ...
    this.persist(this.state);
    // High-frequency writes (e.g. streaming delivery events) pass silent:true
    // and emit their own narrower event, so listeners don't do a full refresh
    // per delta. The state is still persisted above.
    if (!options.silent) {
  ...
  /**
   * Notify subscribers without persisting the full state. For high-frequency
   * events whose durable payload was already written by a preceding mutate().
   * The event is still pushed onto the in-memory ring (bounded) for consumers
   * like the SSE stream, but we skip the expensive full-snapshot write.
   */
