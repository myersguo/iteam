...
import type { Agent, DeliveryWithContext } from "@iteam/shared";
...
const SESSION_IDLE_TTL_MS = readPositiveMs("ITEAM_ACP_SESSION_IDLE_TTL_MS", 5 * 60 * 1000);
const SESSION_GC_INTERVAL_MS = readPositiveMs("ITEAM_ACP_SESSION_GC_INTERVAL_MS", 60 * 1000);
...
export class AcpDriver implements AgentDriver {
  ...
  async start(agent: Agent): Promise<void> {
    ...
    if (!this.sessionGcTimer) {
      this.sessionGcTimer = setInterval(() => this.gcIdleSessions(), SESSION_GC_INTERVAL_MS);
      this.sessionGcTimer.unref();
    }
  }

  private gcIdleSessions(): void {
    const now = Date.now();
    for (const processState of this.processes) {
      if (processState.child.exitCode !== null || processState.child.signalCode !== null) continue;
      for (const [key, session] of processState.sessions) {
        if (key === DEFAULT_SESSION_KEY) continue;
        if (session.activeDeliveryId) continue;
        if (now - session.lastActiveAt < SESSION_IDLE_TTL_MS) continue;
        processState.sessions.delete(key);
        try {
          processState.rpc.notify("session/cancel", { sessionId: session.sessionId });
        } catch {}
      }
    }
  }
  ...
  private async doStart(agent: Agent, processIndex: number): Promise<void> {
    ...
    await rpc.request("initialize", {
      ...
      clientInfo: { name: "iteam-daemon", version: "0.1.45" }
    });
