...
import { parseJsonBody, sendJson, sendText } from "@iteam/shared";
import { HttpError, type IteamCore } from "./core.js";
import { handleAuthRoute, readAuthConfig, requireOAuthSession, type AuthConfig } from "./auth.js";
import type { StoreEvent } from "@iteam/shared";
...
export interface IteamRequestHandler {
  /** The `(req, res)` handler used by the native server. */
  handle(req: IncomingMessage, res: ServerResponse): Promise<void>;
  /** Resolved static config, useful for startup logging. */
  serveWeb: boolean;
  webRoot: string | null;
  /** The live SSE client set (store events fan out to these). */
  readonly sseClients: Set<ServerResponse>;
  /** Resolved auth config used by native routing. */
  readonly authConfig: AuthConfig;
  /** External bot runtime hooks used by native routing. */
  readonly externalBotRuntime?: HttpServerOptions["externalBotRuntime"];
  /** Detach the core store subscription. Call on shutdown. */
  dispose(): void;
}

/**
 * Builds the shared per-request handler (routing + auth + SSE fan-out + static)
 * without owning a socket. `startHttpServer` wraps this in a `node:http` server.
 */
export function createIteamRequestHandler(options: HttpServerOptions): IteamRequestHandler {
  const { core } = options;
  ...
  return { handle, serveWeb, webRoot, sseClients, authConfig, externalBotRuntime: options.externalBotRuntime, dispose: unsubscribe };
}

export function startHttpServer(options: HttpServerOptions): RunningHttpServer {
  const { core, port, host = "127.0.0.1" } = options;
  const handler = createIteamRequestHandler(options);
  const { serveWeb, webRoot, sseClients } = handler;
...
async function route(
  ...
  if (req.method === "GET" && url.pathname === "/api/events") {
    const eventAuth = url.searchParams.get("auth");
    if (eventAuth && !req.headers.authorization) {
      req.headers.authorization = `Bearer ${eventAuth}`;
    }
    res.writeHead(200, {
  ...
  if (req.method === "PATCH" && humanPatch) {
    ...
    if (staticConfig.authConfig.mode === "oauth") {
      const session = requireOAuthSession(staticConfig.authConfig, req);
 ...
 */
export function requireComputerAuth(
  core: IteamCore,
  req: IncomingMessage,
  resourceId: string,
  kind: "agent" | "delivery"
 ...
 */
export function readComputerCredentials(
  req: IncomingMessage,
  url: URL,
 ...
 */
export function resolveSpaceIdHeader(req: IncomingMessage, url: URL): string | null {
...
export function resolveAuthenticatedSpace(
  core: IteamCore,
  req: IncomingMessage,
...
export function resolveMessageAuthorId(
  core: IteamCore,
  authConfig: AuthConfig,
  req: IncomingMessage,
...
export function resolveHumanAuthorId(
  authConfig: AuthConfig,
  req: IncomingMessage,
  authorId: string | null | undefined
): string | undefined {
  if (authConfig.mode !== "oauth") return authorId || undefined;
  const session = requireOAuthSession(authConfig, req);
  const requested = String(authorId || "").trim();
...
export function requireResourceSpaceIfAuthenticated(
  core: IteamCore,
  req: IncomingMessage,
