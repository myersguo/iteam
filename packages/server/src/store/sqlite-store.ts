...
export class SqliteStore extends BaseStore {
  ...
  private runColumnMigrations(): void {
    ...
    this.addColumnIfMissing("iteam_agents", "share_runtime_history", "INTEGER NOT NULL DEFAULT 0");
    ...
    this.addColumnIfMissing("iteam_external_message_links", "external_thread_id", "TEXT");
    this.addColumnIfMissing("iteam_external_message_links", "external_root_message_id", "TEXT");
    this.addColumnIfMissing("iteam_external_message_links", "external_parent_message_id", "TEXT");
    this.addColumnIfMissing("iteam_external_message_links", "external_reply_to_message_id", "TEXT");
  ...
  private migrateAgentsModelNullable(): void {
    ...
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS iteam_agents_new (
        ...
        env                 TEXT,
        share_runtime_history INTEGER NOT NULL DEFAULT 0,
    ...
    this.db.exec(`
      ...
      SELECT id, COALESCE(space_id, 'space_default'), name, handle, description, runtime, model, reasoning, computer_id,
             ...
             updated_at, env, share_runtime_history, last_started_at, last_runtime_status
      FROM iteam_agents
    `);
  ...
  private loadFromTables(): State {
    ...
    const humanRows = this.db
      .prepare("SELECT id, name, handle, role, source, username, email, avatar_url, external_id FROM iteam_humans")
      .all() as Array<{
        id: string;
        name: string;
        handle: string;
