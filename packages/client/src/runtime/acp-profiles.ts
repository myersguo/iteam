...
export function renderAcpProfileArgs(
  ...
  return (profile.args || []).flatMap(arg => {
    if (/^\{\{\s*modelArgs\s*\}\}$/.test(arg)) {
      return params.agent.model ? ["-m", renderModelArg(params.agent)] : [];
    }
    if (/^\{\{\s*modelConfigArgs\s*\}\}$/.test(arg)) {
      return params.agent.model ? ["-c", `model=${JSON.stringify(renderModelArg(params.agent))}`] : [];
    }
...
function renderAcpProfileValue(
  ...
  return value.replace(/\{\{\s*([a-zA-Z0-9_.-]+)\s*\}\}/g, (match, key: string) => {
    switch (key) {
      ...
      case "model":
        return renderModelArg(params.agent);
      case "modelArgs":
        return params.agent.model ? `-m ${renderModelArg(params.agent)}` : "";
...
function renderModelArg(agent: Agent): string {
  const raw = String(agent.model || "").trim();
  if (!raw) return "";
  if (["traex", "trae"].includes(agent.runtime)) return canonicalTraeModelSlug(raw);
  return raw;
}

function canonicalTraeModelSlug(model: string): string {
  return model;
}
