...
export interface OAuthHumanInput {
  id: string;
  name: string;
  handle: string;
  role?: string;
...
export class IteamCore {
  ...
  upsertOAuthHuman(input: OAuthHumanInput): Human {
    ...
    return this.store.mutate<Human>(s => {
      ...
      current.source = input.source || "oauth";
  ...
  createAgent(input: AgentCreateInput): Agent {
    ...
    const agent = this.store.mutate<Agent>(s => {
      ...
      const created: Agent = {
        ...
        updatedAt: now,
        env: input.env || {},
        shareRuntimeHistory: input.shareRuntimeHistory === true
      };
  ...
  patchAgent(agentId: string, input: AgentPatchInput): Agent {
    return this.store.mutate<Agent>(s => {
      ...
      if (input.shareRuntimeHistory !== undefined) current.shareRuntimeHistory = input.shareRuntimeHistory === true;
  ...
  recordRuntimeEvent(agentId: string, input: RuntimeEventInput): void {
    // Transient runtime telemetry (fires per streaming delta); notify listeners
    // without a full-state persist.
    this.store.notify("agent:runtime_event", { agentId, ...input });
  }

  createDeliveryEvent(input: DeliveryEventCreateInput): DeliveryEvent {
    // Streaming events fire per token/tool-call. Persist silently and emit only
    // the narrow `delivery:event` so the web does a light channel refresh
    // instead of a full-workspace refresh (which re-pulls humans/agents/…).
    ...
    this.store.notify("delivery:event", { event });
    return event;
  }
  ...
  createExternalRoutedMessage(input: ExternalRoutedMessageInput): ExternalRoutedMessageResult {
    ...
    const result = this.store.mutate<ExternalRoutedMessageResult>(s => {
      ...
      const externalThreadId = normalizeOptionalString(input.externalThreadId);
      const externalRootMessageId = normalizeOptionalString(input.externalRootMessageId);
      const externalParentMessageId = normalizeOptionalString(input.externalParentMessageId);
      const externalReplyToMessageId = normalizeOptionalString(input.externalReplyToMessageId) || externalMessageId;
      ...
      const sessionScope = externalThreadId || "";
      const sessionBase = `${conversationId}${sessionScope ? `:${sessionScope}` : ""}:${routeTarget}`;
      if (input.asTask) {
        const messageId = this.newId("msg");
        const task: Task = {
          id: this.newId("task"),
        ...
        if (task.assigneeId) {
          this.enqueueSingleAgentDelivery(s, created, task.assigneeId, {
            ...
            depth: 0,
            createdIds,
            sessionKey: normalizeOptionalString(input.sessionKey) || `${sessionBase}:task:${task.id}`,
            source: provider
          });
        }
      ...
      if (finalMentions.length > 0) {
        this.enqueueMentionDeliveries(s, created, {
          ...
          excludeAgentId: null,
          createdIds,
          sessionKey: normalizeOptionalString(input.sessionKey) || sessionBase,
          source: provider
        });
  ...
  private linkOutboundForExternalRoot(s: State, message: Message, rootMessageId: string, now: string): void {
    ...
    s.externalMessageLinks.push({
      ...
      externalRootMessageId: inbound.externalRootMessageId ?? null,
      externalParentMessageId: inbound.externalParentMessageId ?? null,
      externalReplyToMessageId: inbound.externalReplyToMessageId || inbound.externalMessageId || null,
