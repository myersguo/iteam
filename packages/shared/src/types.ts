...
export interface Agent {
  ...
  /**
   * When true, the agent's runtime CLI (traex/claude/codex) inherits the
   * shared per-user home, so its turns appear in that runtime's `/resume`
   * session history. Default false: the runtime is pointed at a private,
   * per-agent home so iteam-driven conversations stay out of the interactive
   * `/resume` picker.
   */
